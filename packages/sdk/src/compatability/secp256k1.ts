...
export class Secp256k1 {
  ...
  public static async makeKeypair(privkey: Uint8Array): Promise<Secp256k1Keypair> {
    ...
    const out: Secp256k1Keypair = {
      ...
      pubkey: Uint8Array.from(keypair.getPublic(false, "array")),
    };
    return out;
  }
...
export function rawSecp256k1PubkeyToRawAddress(pubkeyData: Uint8Array): Uint8Array {
    if (pubkeyData.length !== 65 && pubkeyData.length != 33) {
      throw new Error(`Invalid Secp256k1 pubkey length: ${pubkeyData.length}`);
    }

    const uncompressedPublicKey = Secp256k1.uncompressPubkey(pubkeyData)
    const hash = createKeccakHash('keccak256').update(Buffer.from(uncompressedPublicKey.slice(1))).digest()
    return Uint8Array.from(hash).slice(-20)
}