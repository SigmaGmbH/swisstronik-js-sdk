...
import {
	...
	MinimalImportableIdentitySDKModule
} from './_.js';
...
export const protobufLiterals = {
	MsgCreateDidDoc: "MsgCreateDIDDocument",
	MsgCreateDidDocResponse: "MsgCreateDIDDocumentResponse",
	MsgUpdateDidDoc: "MsgUpdateDIDDocument",
	MsgUpdateDidDocResponse: "MsgUpdateDIDDocumentResponse",
	MsgDeactivateDidDoc: "MsgDeactivateDIDDocument",
	MsgDeactivateDidDocResponse: "MsgDeactivateDIDDocumentResponse",
} as const
...
export interface MsgCreateDidDocEncodeObject extends EncodeObject {
	...
	readonly value: Partial<MsgCreateDIDDocument>
}
...
export interface MsgCreateDidDocResponseEncodeObject extends EncodeObject {
	...
	readonly value: Partial<MsgCreateDIDDocumentResponse>
}
...
export interface MsgUpdateDidDocEncodeObject extends EncodeObject {
	...
	readonly value: Partial<MsgUpdateDIDDocument>
}
...
export interface MsgUpdateDidDocResponseEncodeObject extends EncodeObject {
	...
	readonly value: Partial<MsgUpdateDIDDocumentResponse>
}
...
export interface MsgDeactivateDidDocEncodeObject extends EncodeObject {
	...
	readonly value: Partial<MsgDeactivateDIDDocument>
}
...
export interface MsgDeactivateDidDocResponseEncodeObject extends EncodeObject {
	...
	readonly value: Partial<MsgDeactivateDIDDocumentResponse>
}
...
export type MinimalImportableDIDModule = MinimalImportableIdentitySDKModule<DIDModule>

export type DidExtension = {
	readonly [defaultDidExtensionKey]: {
		readonly didDoc: (id: string) => Promise<DIDDocumentWithMetadataProto>
		readonly didDocVersion: (id: string, versionId: string) => Promise<DIDDocumentWithMetadataProto>
		readonly allDidDocVersionsMetadata: (id: string, paginationKey?: Uint8Array) => Promise<QueryAllDIDDocumentVersionsMetadataResponse>
	}
}
	...
	return {
		[defaultDidExtensionKey]: {
			didDoc: async (id: string) => {
				const { value } = await queryService.DIDDocument({ id })
				assert(value)
				return value
			},
			didDocVersion: async (id: string, versionId: string) => {
				const { value } = await queryService.DIDDocumentVersion({ id, version: versionId })
				assert(value)
				return value
			},
			allDidDocVersionsMetadata: async (id: string, paginationKey?: Uint8Array) => {
				const response = await queryService.AllDIDDocumentVersionsMetadata({ id, pagination: createPagination(paginationKey) as PageRequest | undefined })
				return response
			}
		}
	} as DidExtension
...
export class DIDModule extends AbstractIdentitySDKModule {
	static readonly registryTypes: Iterable<[string, GeneratedType]> = [
        [typeUrlMsgCreateDidDoc, MsgCreateDIDDocument],
        [typeUrlMsgCreateDidDocResponse, MsgCreateDIDDocumentResponse],
        [typeUrlMsgUpdateDidDoc, MsgUpdateDIDDocument],
        [typeUrlMsgUpdateDidDocResponse, MsgUpdateDIDDocumentResponse],
		[typeUrlMsgDeactivateDidDoc, MsgDeactivateDIDDocument],
		[typeUrlMsgDeactivateDidDocResponse, MsgDeactivateDIDDocumentResponse],
    ]
	...
	querier: SwisstronikQuerier & DidExtension

	constructor(signer: SwisstronikSigningStargateClient, querier: SwisstronikQuerier & DidExtension) {
	...
	async createDidDocTx(signInputs: ISignInputs[] | SignInfo[], didPayload: DIDDocumentExternal, address: string, fee?: DidStdFee | 'auto' | number, memo?: string, versionId?: string, context?: IContext): Promise<DeliverTxResponse> {
		if (!this._signer) {
		...
		if(ISignInputs.isSignInput(signInputs)) {
			signatures = await this._signer.signCreateDIDDocumentTx(signInputs, payload)
		} else {
			signatures = signInputs
		}

		const value: MsgCreateDIDDocument = {
			payload,
			signatures
		}
	...
	async updateDidDocTx(signInputs: ISignInputs[] | SignInfo[], didPayload: DIDDocumentExternal, address: string, fee?: DidStdFee | 'auto' | number, memo?: string, versionId?: string, context?: IContext): Promise<DeliverTxResponse> {
		if (!this._signer) {
			this._signer = context!.sdk!.signer
		}
		...
		const payload = MsgUpdateDIDDocumentPayload.fromPartial({
			context: <string[]>didPayload?.['@context'],
			id: didPayload.id,
			controller: <string[]>didPayload.controller,
			verificationMethod: protobufVerificationMethod,
		...
		let signatures: SignInfo[]
		if(ISignInputs.isSignInput(signInputs)) {
			signatures = await this._signer.signUpdateDIDDocumentTx(signInputs, payload)
		} else {
			signatures = signInputs
		}

		const value: MsgUpdateDIDDocument = {
			payload,
			signatures
		}
	...
	async deactivateDidDocTx(signInputs: ISignInputs[] | SignInfo[], didPayload: DIDDocumentExternal, address: string, fee?: DidStdFee | 'auto' | number, memo?: string, versionId?: string, context?: IContext): Promise<DeliverTxResponse> {
		if (!this._signer) {
			this._signer = context!.sdk!.signer
		}
		...
		const payload = MsgDeactivateDIDDocumentPayload.fromPartial({
			id: didPayload.id,
			versionId: versionId
		})
		...
		let signatures: SignInfo[]
		if(ISignInputs.isSignInput(signInputs)) {
			signatures = await this._signer.signDeactivateDIDDocumentTx(signInputs, payload, protobufVerificationMethod!)
		} else {
			signatures = signInputs
		}

		const value: MsgDeactivateDIDDocument = {
			payload,
			signatures
		}
	...
	async queryDidDoc(id: string, context?: IContext): Promise<DIDDocumentWithMetadataExternal> {
		if (!this.querier) {
		...
		} as DIDDocumentWithMetadataExternal
	}

	async queryDidDocVersion(id: string, versionId: string, context?: IContext): Promise<DIDDocumentWithMetadataExternal> {
		if (!this.querier) {
			this.querier = context!.sdk!.querier
		}
	...
	async queryAllDidDocVersionsMetadata(id: string, context?: IContext): Promise<{ didDocumentVersionsMetadata: DIDDocumentMetadata[], pagination: QueryAllDIDDocumentVersionsMetadataResponse['pagination']}> {
		if (!this.querier) {
			this.querier = context!.sdk!.querier
		}
	...
	static async validateSpecCompliantPayload(didDocument: DIDDocumentExternal): Promise<SpecValidationResult> {
	...
	static async toSpecCompliantPayload(protobufDidDocument: DIDDocumentProto): Promise<DIDDocumentExternal> {
