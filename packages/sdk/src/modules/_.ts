...
export abstract class AbstractIdentitySDKModule {
	_signer: SwisstronikSigningStargateClient
	methods: IModuleMethodMap = {}
	querier: SwisstronikQuerier
...
type ProtectedMethods<T extends AbstractIdentitySDKModule, K extends keyof T> = T[K] extends string[] ? T[K][number] : T[K]

export type MinimalImportableIdentitySDKModule<T extends AbstractIdentitySDKModule> = Omit<T, '_signer' | '_protectedMethods' | 'registryTypes' | 'querierExtensionSetup' | 'getRegistryTypes' | 'getQuerierExtensionSetup'>

export function instantiateIdentitySDKModule<T extends new (...args: any[]) => T>(module: T, ...args: ConstructorParameters<T>): T {
	return new module(...args)
}

export function instantiateIdentitySDKModuleRegistryTypes(module: any): Iterable<[string, GeneratedType]> {
    return module.registryTypes ?? []
}

export function instantiateIdentitySDKModuleQuerierExtensionSetup(module: any): QueryExtensionSetup<any> {
