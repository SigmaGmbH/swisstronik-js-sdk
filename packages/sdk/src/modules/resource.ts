...
export type MinimalImportableResourceModule = MinimalImportableIdentitySDKModule<ResourceModule>
...
export type ResourceExtension = {
	readonly [defaultResourceExtensionKey]: {
		...
		readonly resourceMetadata: (collectionId: string, resourceId: string) => Promise<ResourceMetadata>
...
export class ResourceModule extends AbstractIdentitySDKModule {
	...
	querier: SwisstronikQuerier & ResourceExtension

	constructor(signer: SwisstronikSigningStargateClient, querier: SwisstronikQuerier & ResourceExtension) {
	...
	static async signPayload(payload: MsgCreateResourcePayload, signInputs: ISignInputs[] | SignInfo[]): Promise<MsgCreateResource> {
		...
		if(ISignInputs.isSignInput(signInputs)) {
			signatures = await SwisstronikSigningStargateClient.signIdentityTx(signBytes, signInputs)
		} else {
	...
	async queryLinkedResourceMetadata(collectionId: string, resourceId: string, context?: IContext): Promise<ResourceMetadata> {
		if (!this.querier) {
			this.querier = context!.sdk!.querier
		}
