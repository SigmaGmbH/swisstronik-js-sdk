...
export const LitCompatibleCosmosChains = { cosmos: 'cosmos', swisstronik: 'swisstronik' } as const
...
export class LitProtocol {
    ...
    chain: LitCompatibleCosmosChain = LitCompatibleCosmosChains.swisstronik
    private readonly cosmosAuthWallet: Secp256k1HdWallet

    private constructor(options: LitProtocolOptions) {
        // validate options
        if (options.litNetwork && !Object.values(LitNetworks).includes(options.litNetwork)) throw new Error(`[swtr-veramo-plugin]: lit-protocol: Invalid LitNetwork: ${options.litNetwork}`)
        if (options.chain && !Object.values(LitCompatibleCosmosChains).includes(options.chain)) throw new Error(`[swtr-veramo-plugin]: lit-protocol: Invalid LitCompatibleCosmosChain: ${options.chain}`)
        ...
        this.client = function(that: LitProtocol) {
            ...
            throw new Error('[swtr-veramo-plugin]: lit-protocol: Unsupported runtime environment')
        }(this)
    }
    ...
    static async create(options: Partial<LitProtocolOptions>): Promise<LitProtocol> {
        ...
        if (!options?.chain) options.chain = LitCompatibleCosmosChains.swisstronik
    ...
    static async getCosmosWalletPrefix(chain?: LitCompatibleCosmosChain): Promise<string> {
        switch (chain) {
            case LitCompatibleCosmosChains.cosmos:
                return 'cosmos'
            case LitCompatibleCosmosChains.swisstronik:
                return 'swtr'
            default:
                return 'swtr'
        }
    ...
    static async generateTxNonce(format?: TxNonceFormat, entropyLength?: number): Promise<string> {
        switch (format) {
            case TxNonceFormats.entropy:
                return toString(nacl.randomBytes(entropyLength || 64), 'hex')
    ...
    static async generateCosmosAccessControlConditionBalance(returnValueTest: CosmosReturnValueTest, chain: LitCompatibleCosmosChain = LitCompatibleCosmosChains.swisstronik, address = ':userAddress'): Promise<CosmosAccessControlCondition> {
        return {
            conditionType: 'cosmos',
    ...
    static async generateCosmosAccessControlConditionTransactionMemo(returnValueTest: CosmosReturnValueTest, amount: string, sender: string, recipient = ':userAddress', chain: LitCompatibleCosmosChain = LitCompatibleCosmosChains.swisstronik): Promise<CosmosAccessControlCondition> {
        return {
            conditionType: 'cosmos',
    ...
    static async generateCosmosAccessControlConditionInverseTimelock(returnValueTest: CosmosReturnValueTest, amount: string, recipient = ':userAddress', blockHeight = 'latest', chain: LitCompatibleCosmosChain = LitCompatibleCosmosChains.swisstronik): Promise<CosmosAccessControlCondition> {
        return {
            conditionType: 'cosmos',
